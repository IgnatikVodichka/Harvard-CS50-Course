...
def buy():
    ...
    if request.method == "POST":
        ...
        if cash < total:
            ...
            db.execute("INSERT INTO transactions (user_id, symbol, name, shares, price, type) VALUES (?, ? , ?, ?, ?, ?)", user, symbol, stock, shares, price, "buy")
        return redirect("/")
    else:
...
def sell():
    ...
    if request.method == "POST":
        symbol = request.form.get("symbol")
        shares = request.form.get("shares")
        user_shares = db.execute("SELECT SUM(shares) AS shares FROM transactions WHERE (user_id = ? AND symbol = ?)", user, symbol)[0]["shares"]
        ...
        if not shares:
            ...
            cash = db.execute("SELECT cash FROM users WHERE id= ?", user)[0]["cash"]
            db.execute("UPDATE users SET cash = ? WHERE id = ?", cash + total, user)
            db.execute("INSERT INTO transactions (user_id, symbol, name, shares, price, type) VALUES (?, ? , ?, ?, ?, ?)", user, symbol, stock, 0 - shares, price, "sell")
            return redirect("/")
    else:
